#include "./include/functional_page.h"
#include <QTime>
...
void functional_page::draw_ui()
{
    ...
    syn_btn=new QPushButton("语法与静态语义分析");
    ...
    this->painting_borad->hide();
    this->text_browser->show();
}
...
void functional_page::set_btns()
{
    ...
    connect(lex_btn,&QPushButton::clicked,this,[=](){
        ...
        else
        {
           ...
           auto tag=std::get<0>(r);
           auto message=std::get<1>(r);
           auto line=std::get<2>(r);
           auto col=std::get<3>(r);
           if(tag==true)
           {
                is_lex=true;
                output_browser->setText("Lexical Analyse success.");
                QFile f("token_result.tmp");
                f.open(QIODevice::ReadOnly);
                QByteArray content=f.readAll();
                painting_borad->hide();
                text_browser->show();
           ...
           {
                output_browser->setText(QString::fromStdString(message)+" ("+QString::number(line)+","+QString::number(col)+")\n");
           }
    ...
    connect(syn_btn,&QPushButton::clicked,this,[=](){
        ...
        else
        {
            ...
            if(tag==true)
            {
                ...
                output_browser->setText("syntax Analyse success. the holistic precedure will be shown on the right side.\nThe IR file is output as \"IR.txt\" in this directory.");
                draw_syntax_analyse();

            }
            else
            {
                QString str=(QString::fromStdString(message)+" ("+QString::number(line)+","+QString::number(col)+")\n");
    ...
    connect(open_syn_btn,&QPushButton::clicked,this,[=](){
        QString name ="ANSI_YACC_C99.txt";
        QFile f(name);
        f.open(QIODevice::ReadOnly);
        QByteArray content=f.readAll();
        painting_borad->hide();
...
const std::tuple<bool,std::string,int,int> functional_page::lexical_analyse()
{
        LexicalAnalyzer analyzer;

        //读
        std::string in_path =file_name.toStdString();
        ...
        auto re=analyzer.analyze(in_code,out_code,pre_line,pre_col);

        return re;
}

const std::tuple<bool,std::string,int,int> functional_page::syntax_analyse()
{
    Parser parser;
    parser.init();
    parser.read_grammer_Yacc("ANSI_YACC_C99.txt");
    int re=parser.read_cache();
    if(!re) //fail to load the cache
    {
    ...
    auto return_value=parser.check("token_result.tmp");
    //bool tag=(return_value==SYNTAX_SUCCESS);
    return return_value;
}
