...
#include <tuple>
#include "common.h"
extern std::vector<Quaternion> QuadrupleForm;
extern int tempid;
...
private:
    ...
    const std::tuple<bool,std::string,int,int> lexical_analyse();

    const std::tuple<bool,std::string,int,int> syntax_analyse();
